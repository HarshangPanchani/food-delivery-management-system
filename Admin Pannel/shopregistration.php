...
if (isset($_POST['register'])){
    ...
    elseif($password=0){
        ...
        $insert = "INSERT INTO customer_master(`c_pass`, `c_contact_no`, `c_email`, `c_name`) VALUES('$password', '$contact', '$email', '$name')";
        $q=mysqli_query($conn,$insert);
        $insert1 = "INSERT INTO `cust_address_master`(`c_city`, `c_pincode`, `c_area`, `c_address`) VALUES ('$city', '$pincode', '$area', '$address')";
          if(!$q){
            $error = "<script>
            alert ('Email aleready Exsist');
            </script>";
...
<body>
<center>
  <form class="form" action="shopregistration.php" method="post">
  <h2 class="title">Near By FoodCourt</h2>
       <p class="form-title">Register to Shop</p>
       <div class="input-container">
        <input placeholder="Enter ShopName" type="text" name="name" required>
        
    </div>
        <div class="input-container">
          <input placeholder="Enter Shopemail" type="email" name="email" required>
